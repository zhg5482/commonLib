...
abstract class AbstractVideo extends Audio
{
     ...
     * @return array
     */
    protected function basePartOfCommand($format)
    {
        if (isset($format->videoHardDecode)) {
            $prefix = array('-y', '-hwaccel',$format->videoHardware);
            if (!empty($format->videoHardDecode)) {
                $prefix =array_merge($prefix,array('-c:v',$format->videoHardDecode));
            }
            $prefix = array_merge($prefix,array('-i',$this->pathfile));

            return array_merge($prefix,array('-c:v',$format->videoHardEncode));
        }else{
